...
def preprocess_evaluationdata (evaluation_data, pathtomodel):
	# Load ss model
	ss = pickle.load(open(pathtomodel+'scaler_rf.pkl', 'rb'))
	x_predict = ss.transform(evaluation_data)
	#print(x_predict)
	return(x_predict)

def predict_rf(x_predict, evaluation_data, output_data, pathtomodel):
	# Load knn regressor
	rf = pickle.load(open(pathtomodel+'model_rf.pkl', 'rb'))
...
if __name__ == "__main__":	
    #Input arguments to execute rf 
    parser = argparse.ArgumentParser(description='Arguments and data files for executing Nearest Neighbors Regression.')
    parser.add_argument('-m', "--pathtomodel", help='Directory where the rf model will be saved')
    ...
    predict_rf(x_predict, evaluation_data, output_data, pathtomodel)
